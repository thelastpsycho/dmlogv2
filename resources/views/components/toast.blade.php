...
@if($message)
    <div x-data="{ show: true }"
        ...
        <div class="flex items-start gap-3 p-4 rounded-lg shadow-lg border
            @if($type === 'success') bg-accent/20 border-accent/30 text-accent
            @elseif($type === 'error') bg-danger/20 border-danger/30 text-danger
            @elseif($type === 'warning') bg-warning/20 border-warning/30 text-warning
            @else bg-info/20 border-info/30 text-info
            @endif">
